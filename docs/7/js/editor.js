...
`, // Markdown
        ...
        color:'light', // 'light'/'dark'(textareaやhighlight.jsのthemeを一括変更する)
    } }
    #init(options) {
...
//        const srcCnt = this.#textAreaNames.map(k=>this._options[k]).filter(v=>v).length;
    ...
    #colorStyle() {
        ...
        const f = '#FFFFFF';
        const b = '#000000';
        const o = {color:'#FFFFFF', 'background-color':};
        if ('dark'===this._options.color) {return ``}
        */
